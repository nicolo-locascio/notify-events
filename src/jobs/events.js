...
async function insert(data) {
    try {
        ...
        if(bulk.length > 0) {
            let sql = bulk.join(";");
            db.execute(sql).then(() => logger.debug("event sucessfully stored")).catch(e => logger.error("error while inserting event: ", e));
            logger.debug("Insert event finished successfully");
        }
    } catch(err) {
        logger.error("error while preparing event: ", err);
    }
}
